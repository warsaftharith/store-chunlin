  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-line/70 bg-cream/90 backdrop-blur-md">
        <nav className="container-custom flex h-20 animate-pulse items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-2xl bg-line" />
            <div className="h-4 w-32 rounded-full bg-line" />
          </div>

          <div className="hidden items-center gap-7 lg:flex">
            {Array.from({ length: 7 }).map((_, index) => (
              <div key={index} className="h-4 w-16 rounded-full bg-line" />
            ))}
          </div>

          <div className="hidden items-center gap-3 lg:flex">
            <div className="h-10 w-10 rounded-xl bg-line" />
            <div className="h-11 w-20 rounded-2xl bg-line" />
            <div className="h-11 w-32 rounded-2xl bg-line" />
          </div>
      ...
      <section
        id="home"
        className="section-padding min-h-[calc(100vh-80px)] overflow-hidden"
      >
